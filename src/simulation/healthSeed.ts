...
export function initializeHealth(world:WorldState){
  ...
  const year=Number(world.date.slice(0,4));for(const p of world.healthProfiles){if(p.deductibleYear!==year){p.deductibleYear=year;p.deductibleSpentYear=0;}if(p.personId===world.character.id)p.careAccess=clamp(p.careAccess+(hasHealthCoverage(world,p.personId)?1:-1));p.lastUpdatedDate=world.date;}
  return world;
}
