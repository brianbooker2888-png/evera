...
function reconcileEmployerCoverage(world:WorldState){const employment=world.employments.find(e=>e.personId===world.character.id&&e.status==='active'),existing=world.insurancePolicies.find(p=>p.id==='policy-health-player');if(employment?.benefits.healthInsurance){if(existing){existing.ownerIds=[world.character.id];existing.active=true;}else world.insurancePolicies.push({id:'policy-health-player',ownerIds:[world.character.id],kind:'health',provider:'Employer Health Plan',premiumMonthly:145,deductible:1750,coverageLimit:500000,active:true});}else if(existing)existing.active=false;}
function accessTarget(world:WorldState,personId:string){const socio=socioeconomicModifier(world,personId),city=world.cities?.find(c=>c.id===world.currentCityId),country=world.countries?.find(c=>c.id===city?.countryId),local=(city?.healthcareAccess??70)-70,publicBoost=country?.healthcareModel==='universal'?20:country?.healthcareModel==='mixed_public'?9:0,coverageBoost=hasHealthCoverage(world,personId)?country?.healthcareModel==='private_insurance'?28:12:0;return clamp(42+socio*1.5+local*.45+publicBoost+coverageBoost);}
...
export function initializeHealth(world:WorldState,date=world.date){
  initializeLegacyLegal(world,date);
  ...
  const represented=[world.character.id,...world.npcs.filter(n=>n.tier<=2&&n.human&&!isDeceased(world,n.id)).map(n=>n.id)];
