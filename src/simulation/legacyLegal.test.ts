...
describe('Phase 10 mortality, law and legacy',()=>{
  it('keeps legal state inside v11 and migrates a true v8 shape into v11',()=>{
    const current=createWorld(draft,10002);expect(current.version).toBe(11);expect(current.deathRecords).toEqual([]);expect(current.civilCases).toEqual([]);
    ...
    const migrated=migrateWorld({...v8,version:8});expect(migrated?.version).toBe(11);expect(migrated?.healthProfiles.length).toBe(current.healthProfiles.length);expect(migrated?.deathRecords).toEqual([]);expect(migrated?.lifestyleProfiles.length).toBeGreaterThan(0);expect(migrated?.countries.length).toBeGreaterThan(0);
  });
