...
describe('Phase 4 education and careers',()=>{
  it('initializes employment, employers, openings and skills',()=>{
    ...
    expect(world.version).toBe(12);expect(activeEmployment(world)).toBeTruthy();
...
describe('save migration',()=>{
  it('upgrades a v5-shaped save into v12 without losing prior-system history',()=>{
    const current=createWorld(draft,99);
    const{sportsLeagues,sportsTeams,sportsParticipants,athleteProfiles,coachProfiles,sportsContracts,sportsFixtures,sportsSeasonStats,businesses,warehouseFacilities,businessEmployees,logisticsContracts,logisticsKpis,businessOpportunities,healthProfiles,medicalConditions,medicalEncounters,medicalBills,medications,deathRecords,funeralRecords,estatePlans,estateCases,ancestorArchives,controlTransitions,crimeIncidents,criminalCases,criminalRecords,incarcerationRecords,civilCases,lifestyleProfiles,homeLifestyles,homeUpgrades,vehicleUseProfiles,wardrobeItems,hobbies,pets,travelPlans,lifestyleOutings,householdServices,deviceAssets,calendarCommitments,lifestyleMilestones,countries,cities,neighborhoods,industries,worldInstitutions,companyWorldStates,policies,residencyRecords,regionalShocks,worldNews,worldHistory,migrationRecords,currentCityId,currentNeighborhoodId,...phase5Rest}=current;
    void sportsLeagues;void sportsTeams;void sportsParticipants;void athleteProfiles;void coachProfiles;void sportsContracts;void sportsFixtures;void sportsSeasonStats;void businesses;void warehouseFacilities;void businessEmployees;void logisticsContracts;void logisticsKpis;void businessOpportunities;void healthProfiles;void medicalConditions;void medicalEncounters;void medicalBills;void medications;void deathRecords;void funeralRecords;void estatePlans;void estateCases;void ancestorArchives;void controlTransitions;void crimeIncidents;void criminalCases;void criminalRecords;void incarcerationRecords;void civilCases;void lifestyleProfiles;void homeLifestyles;void homeUpgrades;void vehicleUseProfiles;void wardrobeItems;void hobbies;void pets;void travelPlans;void lifestyleOutings;void householdServices;void deviceAssets;void calendarCommitments;void lifestyleMilestones;void countries;void cities;void neighborhoods;void industries;void worldInstitutions;void companyWorldStates;void policies;void residencyRecords;void regionalShocks;void worldNews;void worldHistory;void migrationRecords;void currentCityId;void currentNeighborhoodId;
    const legacy={...phase5Rest,version:5};const migrated=migrateWorld(legacy);
    expect(migrated?.version).toBe(12);expect(migrated?.npcs.length).toBe(current.npcs.length);expect(migrated?.financialAccounts.length).toBeGreaterThan(0);
    expect(migrated?.sportsLeagues.length).toBeGreaterThan(0);expect(migrated?.businessOpportunities.length).toBeGreaterThan(0);
    expect(migrated?.narrationSettings.mode).toBe('offline');expect(migrated?.annualLifeChapters).toEqual([]);expect(migrated?.healthProfiles.length).toBeGreaterThan(0);expect(migrated?.estateCases).toEqual([]);expect(migrated?.lifestyleProfiles.length).toBeGreaterThan(0);expect(migrated?.countries.length).toBeGreaterThan(0);expect(migrated?.gameConfiguration.mode).toBe('life');
  });
});
