...
describe('Phase 4 education and careers',()=>{
  it('initializes employment, employers, openings and skills',()=>{
    ...
    expect(world.version).toBe(10);expect(activeEmployment(world)).toBeTruthy();
...
describe('save migration',()=>{
  it('upgrades a v5-shaped save into v10 without losing prior-system history',()=>{
    const current=createWorld(draft,99);
    const{sportsLeagues,sportsTeams,sportsParticipants,athleteProfiles,coachProfiles,sportsContracts,sportsFixtures,sportsSeasonStats,businesses,warehouseFacilities,businessEmployees,logisticsContracts,logisticsKpis,businessOpportunities,healthProfiles,medicalConditions,medicalEncounters,medicalBills,medications,deathRecords,funeralRecords,estatePlans,estateCases,ancestorArchives,controlTransitions,crimeIncidents,criminalCases,criminalRecords,incarcerationRecords,civilCases,lifestyleProfiles,homeLifestyles,homeUpgrades,vehicleUseProfiles,wardrobeItems,hobbies,pets,travelPlans,lifestyleOutings,householdServices,deviceAssets,calendarCommitments,lifestyleMilestones,...phase5Rest}=current;
    ...
    expect(migrated?.version).toBe(10);expect(migrated?.npcs.length).toBe(current.npcs.length);expect(migrated?.financialAccounts.length).toBeGreaterThan(0);
    expect(migrated?.sportsLeagues.length).toBeGreaterThan(0);expect(migrated?.businessOpportunities.length).toBeGreaterThan(0);
    expect(migrated?.narrationSettings.mode).toBe('offline');expect(migrated?.annualLifeChapters).toEqual([]);expect(migrated?.healthProfiles.length).toBeGreaterThan(0);expect(migrated?.estateCases).toEqual([]);expect(migrated?.lifestyleProfiles.length).toBeGreaterThan(0);
  });
});
