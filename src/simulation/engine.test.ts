...
describe('Phase 4 education and careers',()=>{
  it('initializes employment, employers, openings and skills',()=>{
    ...
    expect(world.version).toBe(11);expect(activeEmployment(world)).toBeTruthy();
...
describe('save migration',()=>{
  it('upgrades a v5-shaped save into v11 without losing prior-system history',()=>{
    ...
    expect(migrated?.version).toBe(11);expect(migrated?.npcs.length).toBe(current.npcs.length);expect(migrated?.financialAccounts.length).toBeGreaterThan(0);
    expect(migrated?.sportsLeagues.length).toBeGreaterThan(0);expect(migrated?.businessOpportunities.length).toBeGreaterThan(0);
    expect(migrated?.narrationSettings.mode).toBe('offline');expect(migrated?.annualLifeChapters).toEqual([]);expect(migrated?.healthProfiles.length).toBeGreaterThan(0);expect(migrated?.estateCases).toEqual([]);expect(migrated?.lifestyleProfiles.length).toBeGreaterThan(0);expect(migrated?.countries.length).toBeGreaterThan(0);
  });
});
