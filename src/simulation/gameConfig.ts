...
export function configurationForMode(mode:GameMode='life',scenarioId:ScenarioId='none',accessibility:AccessibilitySettings=defaultAccessibility,seriousContent:SeriousContentSettings=defaultSeriousContent):GameConfiguration{
    ...
    :mode==='sandbox'
      ...
      :mode==='legacy'
        ...
        :mode==='scenario'
          ...
          :{startingCashMultiplier:1,mortalityMultiplier:1,worldShockMultiplier:1,economicPressureMultiplier:1,allowPrematureDeath:true,revealExtraDecisionContext:false,legacyFocus:false};
  return{mode,scenarioId,rules:{...rules,allowPrematureDeath:rules.allowPrematureDeath&&seriousContent.allowPrematureDeath},seriousContent:{...seriousContent},accessibility:{...accessibility},onboarding:{completed:false,dismissedHints:[]}};
}
...
export function initializeGameConfiguration(world:WorldState):WorldState{
  ...
  world.gameConfiguration.accessibility??={...defaultAccessibility};
  world.gameConfiguration.seriousContent??={...defaultSeriousContent};
  world.gameConfiguration.onboarding??={completed:false,dismissedHints:[]};
  world.gameConfiguration.rules??=configurationForMode(world.gameConfiguration.mode??'life',world.gameConfiguration.scenarioId??'none',world.gameConfiguration.accessibility,world.gameConfiguration.seriousContent).rules;
