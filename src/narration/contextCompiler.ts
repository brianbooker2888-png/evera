...
function eventCategory(type:LifeEvent['type']):NarrativeFact['category']{return type==='finance'?'finance':type==='relationship'?'relationship':type==='health'?'health':type==='world'?'world':'identity';}
...
export function compileSceneContext(world:WorldState,purpose:NarrationPurpose,speakerId:string|null,listenerId:string|null=world.character.id):SceneContextPacket{
  ...
  if(speakerId){
    ...
    for(const condition of world.medicalConditions.filter(c=>c.personId===speakerId&&c.diagnosisStatus==='diagnosed'&&c.status!=='resolved').slice(0,3))pushUnique(facts,canonical(`health-${condition.id}`,'health',`${nameFor(world,speakerId)} has a diagnosed ${condition.name} that is currently ${condition.status}.`,condition.id));
  }
  const recent=world.events.slice(0,8);for(const evt of recent)pushUnique(facts,canonical(`event-${evt.id}`,eventCategory(evt.type),evt.body,evt.id));
...
export function compileAnnualChapterContext(world:WorldState,year:number):SceneContextPacket{
  ...
  for(const event of events)pushUnique(facts,canonical(`event-${event.id}`,eventCategory(event.type),`${event.title}: ${event.body}`,event.id));
  ...
  for(const condition of world.medicalConditions.filter(c=>c.personId===world.character.id&&Number(c.onsetDate.slice(0,4))===year&&c.diagnosisStatus==='diagnosed'))facts.push(canonical(`health-${condition.id}`,'health',`During ${year}, ${condition.name} was diagnosed and became ${condition.status}.`,condition.id));
  const encounters=world.medicalEncounters.filter(e=>e.personId===world.character.id&&Number(e.date.slice(0,4))===year);if(encounters.length)facts.push(canonical(`healthcare-year-${year}`,'health',`During ${year}, the character had ${encounters.length} recorded healthcare encounter${encounters.length===1?'':'s'}.`,`healthcare-year-${year}`));
