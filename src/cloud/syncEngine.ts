...
export async function syncWorld(world:WorldState,provider:CloudSaveProvider):Promise<SyncResult>{
  ...
  try{
    ...
    const remoteWorld=validRemote(remote);
    if(remote.checksum===localChecksum){meta=syncedMeta(meta,remote);await saveSyncMetadata(meta);return{kind:'synced',world,metadata:meta,direction:'none'};}
    const localChanged=meta.lastSyncedChecksum===null||localChecksum!==meta.lastSyncedChecksum;
    const remoteChanged=meta.lastSyncedChecksum===null||remote.checksum!==meta.lastSyncedChecksum;
    if(!localChanged&&remoteChanged){meta=syncedMeta(meta,remote);await replaceWorldFromSync(remoteWorld,meta);return{kind:'synced',world:remoteWorld,metadata:meta,direction:'download'};}
    if(localChanged&&!remoteChanged){const pushed=await provider.push(localEnvelope(world,meta,remote.revision+1),remote.revision);meta=syncedMeta(meta,pushed);await saveSyncMetadata(meta);return{kind:'synced',world,metadata:meta,direction:'upload'};}
