...
export function SportsMatchViewer({world,fixture,playerTeamId}:{world:WorldState;fixture:SportsFixture;playerTeamId?:string|null}){
  ...
  return <article className="match-room">
    ...
    {mode==='result'?<div className="result-only"><Flag/><h3>{fixture.homeScore===fixture.awayScore?'Draw':fixture.homeScore>fixture.awayScore?`${home} won`:`${away} won`}</h3><p>The result comes directly from the deterministic match engine. Switch viewing modes to replay stored moments.</p></div>:<>
      <div className={`match-surface ${fixture.sport==='soccer'?'soccer':'football'}`} aria-label={`${fixture.sport==='soccer'?'Soccer pitch':'American football field'} visualization`}>
        ...
        {current&&<div className={`moment-marker ${current.teamId===fixture.homeTeamId?'home':'away'} ${playerMoment?'player':''}`} style={{left:`${8+progress*84}%`,top:`${lane}%`}}><span>{current.type==='score'?<Goal/>:<Flag/>}</span></div>}
      </div>
      <div className="moment-stage">{current?<><div className="moment-meta"><span className="eyebrow">{momentClock(fixture,current)} · {eventLabel(current).toUpperCase()}</span><b>{current.teamId===fixture.homeTeamId?home:away}</b></div><h3>{current.description}</h3><div className="importance-track"><span style={{width:`${current.importance}%`}}/></div><small>Importance {Math.round(current.importance)}/100 · moment {cursor+1} of {moments.length}</small>{playerMoment&&playerInMatch&&<div className="player-moment"><VisualPerson world={world} personId={world.character.id} date={fixture.date} context="athletic" compact caption="You were directly involved in this stored match moment."/></div>}</>:<p className="muted">No stored moments match this viewing mode.</p>}</div>
      <footer className="replay-controls"><button type="button" className="secondary-btn" disabled={!moments.length||cursor===0} onClick={()=>setCursor(c=>Math.max(0,c-1))}><ChevronLeft/>Previous</button><button type="button" className="primary mini" disabled={moments.length<2} onClick={()=>setPlaying(p=>!p)}>{playing?<><CirclePause/>Pause</>:<><CirclePlay/>Play</>}</button><button type="button" className="secondary-btn" disabled={!moments.length||cursor>=moments.length-1} onClick={()=>setCursor(c=>Math.min(moments.length-1,c+1))}>Next<ChevronRight/></button><button type="button" className="icon-button" aria-label="Restart replay" disabled={!moments.length} onClick={()=>{setCursor(0);setPlaying(false);}}><RotateCcw/></button></footer>
    </>}
  </article>;
}
