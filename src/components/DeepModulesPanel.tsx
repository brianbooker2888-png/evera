...
import { SportsMatchViewer } from './SportsMatchViewer';
...
function SportsPanel({world,onChange}:{world:WorldState;onChange:(world:WorldState)=>void}){
  ...
  return <Section title="Sports world" subtitle="Soccer and American football use one deterministic match engine. Visual presentation reads the stored match state rather than inventing a second result.">
    {!profile?<>
      ...
      <div className="sports-identity-grid"><VisualPerson world={world} personId={world.character.id} context="athletic" caption={athlete?`${athlete.position} for ${team?.name??'your team'}`:`Head coach of ${team?.name??'your team'}`}/><div>
      ...
      {nextMatch&&<article className="fixture-card next-match-room"><span className="eyebrow">NEXT MATCH · {fmtDate(nextMatch.date)}</span><h3>{teamName(world,nextMatch.homeTeamId)} vs {teamName(world,nextMatch.awayTeamId)}</h3><p>Prepare before the fixture. These choices change canonical athlete readiness or coaching approach before the deterministic match is simulated.</p>{athlete?<div className="module-actions"><button className="secondary-btn" onClick={()=>onChange(trainSport(world,athlete.sport,'skills'))}><Dumbbell size={15}/>Sharpen skills</button><button className="secondary-btn" onClick={()=>onChange(trainSport(world,athlete.sport,'fitness'))}><Activity size={15}/>Push fitness</button><button className="secondary-btn" onClick={()=>onChange(trainSport(world,athlete.sport,'recovery'))}>Prioritize recovery</button></div>:coach?<div className="module-actions"><button className={coach.style==='aggressive'?'primary mini':'secondary-btn'} onClick={()=>onChange(setCoachStyle(world,coach.sport,'aggressive'))}>Aggressive</button><button className={coach.style==='balanced'?'primary mini':'secondary-btn'} onClick={()=>onChange(setCoachStyle(world,coach.sport,'balanced'))}>Balanced</button><button className={coach.style==='conservative'?'primary mini':'secondary-btn'} onClick={()=>onChange(setCoachStyle(world,coach.sport,'conservative'))}>Conservative</button></div>:null}</article>}
      {lastMatch&&<SportsMatchViewer world={world} fixture={lastMatch} playerTeamId={team?.id}/>} 
    </>}
