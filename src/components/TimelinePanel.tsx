import { BookOpen, History, Sparkles, UsersRound } from 'lucide-react';
...
function archivedName(world:WorldState,id:string){return world.ancestorArchives.find(a=>a.personId===id)?.name??world.npcs.find(n=>n.id===id)?.name??(id===world.character.id?`${world.character.firstName} ${world.character.lastName}`:'Unknown person');}

export function TimelinePanel({world,onChange}:{world:WorldState;onChange:(world:WorldState)=>void}){
  const personal=world.character.human.memories.slice().sort((a,b)=>b.date.localeCompare(a.date)).slice(0,10),ancestors=world.ancestorArchives.slice().sort((a,b)=>b.deathDate.localeCompare(a.deathDate)),transitions=world.controlTransitions.slice().sort((a,b)=>b.date.localeCompare(a.date));
  return <>
